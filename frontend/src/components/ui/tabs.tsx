...
export function Tabs({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('inline-flex rounded-md border border-slate-300 p-1', className)} {...props} />;
}
...
export function TabsTrigger({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={cn('rounded px-3 py-1.5 text-sm hover:bg-slate-100', className)} {...props} />;
}
