...
export class CacheInvalidationListener {
  ...
  async onPaymentCreated(event: PaymentCreatedEvent): Promise<void> {
    await this.cacheService.del(`dashboard:summary:${event.tenantId}:${event.branchId}`);
    await this.cacheService.del(`dashboard:overdue:${event.tenantId}:${event.branchId}`);
  }
  ...
  async onTransferCreated(event: TransferCreatedEvent): Promise<void> {
    await this.cacheService.del(`reports:cashflow:${event.tenantId}:${event.branchId}`);
  }
}
