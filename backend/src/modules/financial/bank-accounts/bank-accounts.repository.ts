...
export class BankAccountsRepository {
  ...
  async findById(id: string, branchId: string): Promise<BankAccountEntity | null> {
    const schema = quoteIdent(this.drizzleService.getTenantSchema());
  ...
  async create(branchId: string, dto: CreateBankAccountDto): Promise<BankAccountEntity> {
    const schema = quoteIdent(this.drizzleService.getTenantSchema());
  ...
  async update(
    ...
    if (dto.bankCode !== undefined) sets.push(`bank_code = ${quoteLiteral(dto.bankCode)}`);
    if (dto.agency !== undefined) sets.push(`agency = ${quoteLiteral(dto.agency)}`);
    if (dto.accountNumber !== undefined) sets.push(`account_number = ${quoteLiteral(dto.accountNumber)}`);
    if (dto.type !== undefined) sets.push(`type = ${quoteLiteral(dto.type)}`);
    if (dto.initialBalance !== undefined) sets.push(`initial_balance = ${quoteLiteral(dto.initialBalance)}`);
