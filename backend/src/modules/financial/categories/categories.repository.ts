...
export class CategoriesRepository {
  ...
  async update(id: string, branchId: string, dto: { name?: string; type?: string; parentId?: string; color?: string; sortOrder?: number }): Promise<CategoryEntity> {
    const schema = quoteIdent(this.drizzleService.getTenantSchema());
    const idLiteral = quoteLiteral(id);
    const branchLiteral = quoteLiteral(branchId);
    const sets: string[] = [];
    ...
    if (dto.parentId !== undefined) sets.push(`parent_id = ${quoteLiteral(dto.parentId)}`);
    if (dto.color !== undefined) sets.push(`color = ${quoteLiteral(dto.color)}`);
