...
export class EntryRules {
  validateCreate(dto: CreateEntryDto, lockedUntil: Date | null): void {
    if (Number(dto.amount) <= 0) {
      throw new BusinessException('VALIDATION_ERROR', 'Valor deve ser positivo', {
        field: 'amount',
      }, 400);
    }

    if (dto.dueDate < dto.issueDate) {
      throw new BusinessException('VALIDATION_ERROR', 'Vencimento deve ser >= emissao', {
        dueDate: dto.dueDate,
    ...
    if (lockedUntil && new Date(dto.issueDate) <= lockedUntil) {
      throw new BusinessException(
        ...
        { lockedUntil: lockedUntil.toISOString().slice(0, 10), entryDate: dto.issueDate },
      );
    }
  }
}
