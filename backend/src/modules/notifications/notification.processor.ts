    ...
    this.queueService.registerProcessor(
      ...
      async (rawPayload) => {
        try {
          ...
          const schema = resolveTenantSchema(rawPayload);
          ...
          if (!jobType) {
            this.logger.warn(
              'Skipping notification: missing jobType in payload',
            );
            return;
          }
            ...
            case 'entry.due_soon':
              title =
                entryType === 'PAYABLE'
                  ...
                  : 'Conta a receber vence em breve';
          ...
          this.logger.error(
            `Erro ao processar notification job: ${e instanceof Error ? e.stack : String(e)}`,
          );
        }
      },
    );
